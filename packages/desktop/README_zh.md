...
完整桌面窗口包含自定义标题栏、常驻侧边导航、npm/GitHub 输入框、三级风险图例和带证据的风险卡。

![npm-safe 桌面端完整安装风险页面](docs/desktop-preflight.png)

### 本地安全总览

总览页把本地持久化的扫描历史转换成安全仪表、近 7 日活动、风险分布和最近结果。

![npm-safe 桌面端完整总览页面](docs/desktop-overview.png)
