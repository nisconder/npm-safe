...
界面使用三种明确结论，让用户无需阅读原始扫描日志，就能区分“可以隔离测试”“安装前审查”和“修复后再安装”。

![低、中、高三种 DSH 安装风险状态](../../docs/assets/dsh-risk-states.svg)

详细风险卡把固定来源、7 项证据、问题建议和仅复制的安装命令收进同一张检查单。

![DSH 插件安装详细风险卡](../../docs/assets/dsh-install-risk-card.svg)

<sub>界面预览使用示例包数据；生成风险卡时，应用不会执行安装。</sub>
